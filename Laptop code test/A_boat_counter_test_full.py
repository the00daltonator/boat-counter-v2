...
VIDEO_SOURCE = "test_boats4.mp4"  # Path to the input video file
MODEL_PATH = "yolov8n.pt"         # Path to YOLOv8 model file
CLASS_FILTER = "boat"             # Only detect and count boats
CONFIDENCE_THRESHOLD = 0.15        # Minimum confidence for detection 0.15
...
while True:  # Loop over video frames
    ...
    for result in resultsTracker:  # Loop over tracked objects
        ...
        if len(id_history[id]) >= 2:  # If enough history to check movement
            ...
            # Check if the line was crossed in either direction
            crossed = any(
            ...
            if crossed and distance > 15 and id not in counted_ids:  # If crossed, moved enough, and not counted
        ...
        # === DRAWING ===
        # Draw bounding box, ID, direction, and trajectory for each tracked object
        if id in id_history and len(id_history[id]) >= 2:
