...
VIDEO_SOURCE      = 0 #"test_pattern.mp4" 0 //  camera index or video file
    ...
    try:
        while True:
            ...
            if len(detections) == 0:
                ...
                tracks = tracker.update(np.array(detections, dtype=np.float32))  # Update tracker # tracks = tracker.update(np.array(detections, dtype=np.float32))  # Update tracker
